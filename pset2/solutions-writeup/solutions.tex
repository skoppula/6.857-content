...
b. XOR'ing two inputs does not change one-way of the function. $x \oplus y$ distributes evenly across the input space $\{0,1\}^d$ of $h(n)$; if finding $n$ given $h(n)$ is is worst case $O(2^{d})$ operations, finding $x \oplus y$ (and thus $x,y$) given $h'(x,y)=h(x\oplus y)$, is still $O(2^d)$.\\

However, in the case of AND'ing the two inputs, $x \and y$
